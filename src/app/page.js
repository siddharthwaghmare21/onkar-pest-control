...
export default function Home() {
  return <>
    ...
    <main>
      ...
      <section className="areas-section"><div className="container areas-grid"><div><div className="section-kicker">WHERE WE SERVE <span /></div><h2>Care that travels<br /><em>with you.</em></h2><p>सांगलीपासून आसपासच्या शहरांपर्यंत residential आणि commercial pest control services.</p></div><div className="area-content"><div className="area-tags">{company.serviceAreas.map((area) => <span key={area}><MapPin size={14} />{area}</span>)}</div><p className="area-note"><Clock3 size={16} /> {company.workingHours} &nbsp; · &nbsp; Locations outside Sangli may attract additional travelling charges.</p></div></div></section>
